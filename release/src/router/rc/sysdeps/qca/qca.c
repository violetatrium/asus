...
#if defined(RTCONFIG_LYRA_5G_SWAP)
	org_band=band;
	swap_band=swap_5g_band(band);
	if(band!=swap_band)
	{
		max_subnet = num_of_mssid_support(swap_band);
		band=swap_band;
	}
#endif
...
//			&& nvram_invmatch(strcat_r(prefix_wlc, "ssid", tmp), "")&& subnet==0)
			&& subnet==0)
#else
#if defined(RTCONFIG_REPEATER_SELECT5G)
	if (sw_mode==SW_MODE_REPEATER && nvram_invmatch("wlc_ssid", "") && subnet==0
		&& ((band==WL_5G_BAND && wlc_band==WL_5G_2_BAND) || (band==WL_5G_2_BAND && wlc_band==WL_5G_BAND) || wlc_band==band))
#else
...
//			band,nvram_safe_get(strcat_r(prefix_wlc, "ssid", tmp)));
	...
	if (band == 0 && nvram_match(strcat_r(tmpfix, "country_code", tmp), "GB") 
	    && (nvram_match("territory_code", "EU/02") || nvram_match("territory_code", "UK/02")))
	{ /* disable 2G txbf when using 5V for 2G PA in GB */
		txbf = 0;
		mumimo = 0;
	}
#endif
	...
	if(!strcmp(wif,WIF_5G_BH))
	{
		char part1[10],part2[10];
		fprintf(fp2,"iwpriv %s hide_ssid 1\n",wif);
		memset(part1,0,sizeof(part1));
		memset(part2,0,sizeof(part2));
		get_mesh_ie(part1,part2);
		fprintf(fp2,"iwpriv %s mesh_info 0x%s 0x%s\n",wif,part1,part2);
	}
#endif
...
#if !defined(RTCONFIG_SOC_IPQ40XX) && !defined(MAPAC1750) || defined(VZWAC1300)
					//ext ch
	...
	{
#if defined(RTCONFIG_LYRA_5G_SWAP)
                fprintf(fp3, "iwpriv %s dcs_enable 0\n", vphy); //not to scan and change to other channels
#else
...
#if defined(VZWAC1300) || defined(MAPAC1300) || defined(MAPAC1750) /* by PM: lock at band 4 */
		if(band) //5G
...
#ifndef RTCONFIG_DUAL_BACKHAUL
	if(sw_mode() != SW_MODE_REPEATER && unit==0)
	{
...
char *getWscStatus_enrollee(int unit, char *buf, int buflen)
{
	...
	snprintf(cmdbuf, sizeof(cmdbuf), "wpa_cli -i %s status", get_staifname(unit));
	fp = popen(cmdbuf, "r");
	if (fp) {
		memset(buf, 0, buflen);
...
#if defined(RTCONFIG_WIFI_SON)&& defined(RTCONFIG_HIDDEN_BACKHAUL)
#define target 10
char str[target][40]={"Address:","ESSID:","Frequency:","Quality=","Encryption key:","IE:","Authentication Suites","Pairwise Ciphers","phy_mode=","mesh_ie"};
#else 
#define target 9
...
getSiteSurvey(int band,char* ofile)
{
	...
	if (band < 0 || band >= MAX_NR_WL_IF)
        ...
        sprintf(tmp_mesh,"0x%s%s",part1,part2);
#endif
...
skip_init:
	...
	while(1)
	{
		...
		memset(mesh,0,sizeof(mesh));
	        pt1 = strstr(buf[9], "mesh_ie");	
		if(pt1)
			strncpy(mesh,pt1+strlen("mesh_ie="),strlen(buf[9])-(pt1+strlen("mesh_ie=")-buf[9])-1);
#endif
#endif
...
#else
	unit=nvram_get_int("wlc_band");
#if defined(RTCONFIG_WIRELESSREPEATER) && defined(RTCONFIG_REPEATER_SELECT5G)
	if (sw_mode()==SW_MODE_REPEATER && (unit==WL_5G_BAND || unit==WL_5G_2_BAND)) {
		ret=getPapState(unit^3);
		if(ret==2) { //connected
			nvram_set_int("wlc_band", unit^3);
			nvram_commit();
			wpa_supplicant_stop(unit);
			return ret;
		}
	}
#endif
...
#if defined(RTCONFIG_WIRELESSREPEATER) && defined(RTCONFIG_REPEATER_SELECT5G)
	else if (sw_mode()==SW_MODE_REPEATER && (unit==WL_5G_BAND || unit==WL_5G_2_BAND) && ret==2)
		wpa_supplicant_stop(unit^3);
#endif
...
void gen_spcmd(char *cmd)
{
	...
	f_write_string(CFG_JSON_FILE, "{\"spcmd\":\"\"}", 0, 0);
	/* trigger */
...
#if defined(MAPAC1300) || defined(MAPAC2200) || defined(MAPAC1750) || defined(MAPAC2200P)
// apply general rule
	char *phy;
	if (strcmp(country, "GB")==0) {
		eval("iwpriv", VPHY_2G, "acs_weight", "12");
		eval("iwpriv", VPHY_2G, "acs_weight", "132");
	} else {
		eval("iwpriv", VPHY_2G, "acs_weight", "13");
		eval("iwpriv", VPHY_2G, "acs_weight", "114");
	}

	phy = VPHY_5G;
	do {
		eval("iwpriv", phy, "acs_weight", "403");
		eval("iwpriv", phy, "acs_weight", "443");
		if (strcmp(country, "GB")==0) {
			eval("iwpriv", phy, "acs_weight", "363");
			eval("iwpriv", phy, "acs_weight", "483");
		} else {
			eval("iwpriv", phy, "acs_weight", "366");
			eval("iwpriv", phy, "acs_weight", "486");
		}
		eval("iwpriv", phy, "acs_weight", "523");
		eval("iwpriv", phy, "acs_weight", "563");
		eval("iwpriv", phy, "acs_weight", "603");
		eval("iwpriv", phy, "acs_weight", "643");
		eval("iwpriv", phy, "acs_weight", "1653");
#if defined(MAPAC2200) || defined(MAPAC2200P) // tri-band
		if (strcmp(phy, VPHY_5G2)!=0) {
			phy = VPHY_5G2;
			continue;
		}
#endif
