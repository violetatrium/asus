...
#if defined(MAPAC2200) || defined(MAPAC2200P)
...
char *get_stamac(int band, char *buf, int blen)
{
        FILE *fp;
        int len;
        char *pt1,*pt2;
...
static void gen_wsplcd_conf(int role, char *filename, int cmp_only)
{
...
#if defined(RTCONFIG_HIDDEN_BACKHAUL)
	char vlan_wif_bh[20],vlan_sta_bh[20];
	sprintf(vlan_wif_bh,"%s.1",WIF_5G_BH);
	sprintf(vlan_sta_bh,"%s.1",STA_5G);
#endif
	...
	foreach(word, nvram_safe_get("lan_ifnames"), next_word){
		if (strstr(word, "ath") && strlen(word) < 5/* ignore guest network */) {
			if (strlen(tmp1) != t1)
				sprintf(tmp1, "%s,", tmp1);
			sprintf(tmp1, "%s%s:WLAN", tmp1, word);
		}
		if (strstr(word, "eth") || strstr(word, "vlan")) {
			if(strlen(tmp2) != t2)
				sprintf(tmp2, "%s,", tmp2);
	...
	fprintf(fp,"%s\n",tmp1);
#if defined(RTCONFIG_HIDDEN_BACKHAUL)
	sprintf(tmp2, "%s,%s:WLAN_VLAN",tmp2,vlan_wif_bh);
	if(role)
		sprintf(tmp2, "%s,%s:WLAN_VLAN",tmp2,vlan_sta_bh);
#endif
	fprintf(fp,"%s\n",tmp2);
...
#if defined(RTCONFIG_HIDDEN_BACKHAUL)
	fprintf(fp, "deep_clone_no_bssid=0\n");
#else
...
#ifdef RTCONFIG_DUAL_BACKHAUL
	tmp_pt = get_stamac(0, stamac, sizeof(stamac));
	if(role)
		fprintf(fp, "WLAN.3.BSSID=%s\n", tmp_pt? tmp_pt:""); //self mac
	else
#endif
	...
	tmp_pt = get_stamac(1, stamac, sizeof(stamac));
	if(role)
...
#if defined(RTCONFIG_HIDDEN_BACKHAUL)
	sprintf(tmp1, "%s,%s:WLAN_VLAN",tmp1,vlan_wif_bh);
	if(role)
		sprintf(tmp1, "%s,%s:WLAN_VLAN",tmp1,vlan_sta_bh);
#endif
	fprintf(fp,"%s\n",tmp1);
	if(role)
	{
...
#if defined(RTCONFIG_QCA953X) || defined(RTCONFIG_QCA956X)
	fprintf(fp, "[MONITOR]\n");
	fprintf(fp, "DisableMonitoringLegacyClients=1\n");
	fprintf(fp, "DisableSteeringInactiveLegacyClients=1\n");
	fprintf(fp, "DisableSteeringActiveLegacyClients=1\n");
	fprintf(fp, "DisableSteeringMax11kUnfriendlyClients=1\n");
	fprintf(fp, "MonitorTimer=60\n");
	fprintf(fp, "MonitorResponseTimeout=5\n");
#endif
...
#if defined(RTCONFIG_HIDDEN_BACKHAUL)
        if(!strstr(tmp1, WIF_5G_BH))
		sprintf(tmp1, "%s,wifi1:%s",tmp1,WIF_5G_BH);
#endif
	fprintf(fp,"%s\n",tmp1);
#if defined(RTCONFIG_QCA953X) || defined(RTCONFIG_QCA956X)
...
#if defined(RTCONFIG_QCA953X) || defined(RTCONFIG_QCA956X)
	fprintf(fp, "BlacklistOtherESS=0\n");
#endif
...
#if defined(RTCONFIG_QCA953X) || defined(RTCONFIG_QCA956X)
	fprintf(fp, "Max11kUnfriendly=10\n");
#endif
...
#if defined(RTCONFIG_QCA953X) || defined(RTCONFIG_QCA956X)
	fprintf(fp, "ApplyEstimatedAirTimeOnSteering=1\n");
#endif
#if defined(MAPAC1750)
	if(role)
		fprintf(fp,"APSteerToRootMinRSSIIncThreshold=4\n");
	fprintf(fp, "APSteerToLeafMinRSSIIncThreshold=5\n");
	if(role)
		fprintf(fp,"APSteerToPeerMinRSSIIncThreshold=5\n");
#else
	if(role)
...
void set_hops_count(int specific)
{
	...
	{
#if 0 /* forced Star-Topology */
		doSystem("iwpriv %s set_whc_dist %d", get_wififname(0), 255);
		doSystem("iwpriv %s set_whc_dist %d", get_wififname(1), 255);
#else
...
#endif	
#if defined(RTCONFIG_HIDDEN_BACKHAUL)
		doSystem("iwpriv %s set_whc_dist %d", WIF_5G_BH,distance);
#endif
	}
...
void set_vap(int role, int band)
{
...
#if defined(RTCONFIG_HIDDEN_BACKHAUL)
	if(sw_mode() != SW_MODE_REPEATER && band)
	{
		doSystem("iwpriv %s rrm 1",WIF_5G_BH);
		doSystem("iwpriv %s wds 1",WIF_5G_BH);
		doSystem("iwpriv %s extap 0",WIF_5G_BH);
		doSystem("iwpriv %s blockdfschan %d",WIF_5G_BH, block_dfs_enable);
		doSystem("iwpriv %s son 1",WIF_5G_BH);
		doSystem("iwpriv %s mcastenhance 5",WIF_5G_BH);
		doSystem("iwpriv %s hide_ssid 1",WIF_5G_BH);
		//doSystem("iwpriv %s rept_spl 1",WIF_5G_BH);
	}
	...
	{
		if(role) //range extender
			doSystem("iwpriv %s set_whc_dist 255",WIF_5G_BH);
		else
			doSystem("iwpriv %s set_whc_dist 0",WIF_5G_BH);
	}
#else
...
#if defined(RTCONFIG_HIDDEN_BACKHAUL)
		//if(sw_mode() != SW_MODE_REPEATER && band)
			//doSystem("iwpriv %s  rept_spl 1", get_staifname(band));
#endif
...
#if defined(MAPAC2200) /* for dpdt_ant */
...
//////// release binding in sta & ath
		doSystem("iwpriv %s athnewind 1", get_wififname(1));
#ifdef RTCONFIG_DUAL_BACKHAUL
		doSystem("iwpriv %s athnewind 1", get_wififname(0));
#endif
////////
	...
	else if(c==0) //RE LAN down
	{
		...
		set_hy_mcast();
#endif
//////// create binding in sta & ath
		doSystem("iwpriv %s athnewind 0", get_wififname(1));
#ifdef RTCONFIG_DUAL_BACKHAUL
		doSystem("iwpriv %s athnewind 0", get_wififname(0));
#endif
////////
...
#if defined(MAPAC1300) || defined(MAPAC2200) || defined(VZWAC1300) || defined(MAPAC1750) || defined(MAPAC2200P)
		if (IPTV_ports_cnt() >= 2)
		...
		if (!role) { //CAP
			if (hb_count_down <= 0) {
				...
				if (count == 0) { /* single CAP case, enable wifi0, wifi2 NCB */
					if (nvram_get_int("ncb_enable") < 2) { /* prevent to kick client in all bands */
						doSystem("iwpriv wifi0 ncb_enable 5138"); /* 0x1412 */
						doSystem("iwpriv wifi2 ncb_enable 5138"); /* 0x1412 */
					}
...
#if defined(RTCONFIG_HIDDEN_BACKHAUL) 
void update_mesh_up_sta(void)
{
#ifdef RTCONFIG_ETHBACKHAUL
	int i;
	unsigned char up_tmp[EXCHANGE_MAC_LEN+5+1], up_mac[EXCHANGE_MAC_LEN+1];
	strcpy(up_tmp, nvram_safe_get("wl1_sta_bssid"));
	for ( i=0; i<6; i++) {
...
#endif //RTCONFIG_HIDDEN_BACKHAUL
...
#if defined(RTCONFIG_HIDDEN_BACKHAUL)
void add_bh_network(void)
{
	char buf[128],*pt=NULL;
        if(strlen(nvram_safe_get("cfg_group")))
        {
                pt=nvram_safe_get("lan_ifnames");
                if (pt) {
                        if(!strstr(pt,WIF_5G_BH))
                        {
                                sprintf(buf,"%s %s",nvram_safe_get("lan_ifnames"),WIF_5G_BH);
                                nvram_set("lan_ifnames",buf);
                                _dprintf("==> add %s to br0 temporarily!!!\n",WIF_5G_BH);
                        }
                        else
                                _dprintf("lan_ifnames=%s\n",pt);
                }
        }
}

void del_bh_network(void)
{
	if(strlen(nvram_safe_get("cfg_group")))
        {
                char *tmp_str = strdup(nvram_safe_get("lan_ifnames"));
                if (tmp_str) {
                        if (remove_word(tmp_str, WIF_5G_BH)) {
                                trim_space(tmp_str);
                                nvram_set("lan_ifnames", tmp_str);
                                nvram_commit();
                                eval("brctl", "delif", nvram_safe_get("lan_ifname"), WIF_5G_BH);
                                _dprintf("==> del %s from br0!!!\n",WIF_5G_BH);
                        }
                        free(tmp_str);
                }
        }
...
int check_bh(char* iface)
{
	char *pt=NULL;
	char vlan_bh[25];
        sprintf(vlan_bh,"%s.1",iface);
        if(strlen(nvram_safe_get("cfg_group")))
        {
                pt=nvram_safe_get("lan_ifnames");
                if (pt) 
		{
                	if(!strstr(pt,vlan_bh))
                        	return 1;
		}
        }
	return 0;
...
void addXOR(char *plaintext, char *ciphertext)
{
        int i;
	int k=strlen(meshkey);
        for(i=0;i<strlen(plaintext);i++)
                *(ciphertext+i)=meshkey[i%k]^(*(plaintext+i));
        *(ciphertext+strlen(plaintext))='\0';
}

void delXOR(char* ciphertext,int cipher_lens,char* plaintext)
{
        int i;
        int k=strlen(meshkey);
        for(i=0;i<cipher_lens;i++)
        {
                *(plaintext+i)=meshkey[i%k]^(*(ciphertext+i));
        }
        *(plaintext+cipher_lens)='\0';
}

void write_hash_file(char *src,int lens)
{
        FILE *fp;
        int i;

	unlink("/tmp/tmp_hash");
        fp=fopen("/tmp/tmp_hash","w");
        if(fp!=NULL)
        {
                for(i=0;i<lens;i++)
                {
...
void read_hash_file(char *src,int *lens)
{
        FILE *fp;
        int len,i;
        char buf[100],tmp[5];
        fp = fopen("/tmp/tmp_hash", "r");
        if (fp) {
               memset(buf, 0, sizeof(buf));
               len = fread(buf, 1, sizeof(buf), fp);
               fclose(fp);
 ...
 //option: 0/1/2
char* gen_hash_string(int option,char* src,int lens)
{
	FILE *fp;
	int len;
	int i;
	...
	if(src)
	{
		write_hash_file(src,lens);
        	sprintf(buf, "md5sum /tmp/tmp_hash | cut -b %d-%d",10*option+1,10*(option+1));
        	fp = popen(buf, "r");
        	if (fp) {
                	memset(buf, 0, sizeof(buf));
                	len = fread(buf, 1, sizeof(buf), fp);
...
void setting_hash_ap(char* src,int lens)
{
	char *ssid=NULL,*psk=NULL;
	char tmp[128];
	if ((ssid=gen_hash_string(0,src,lens))) 
	{
		sprintf(tmp,"%s-%s",SSID_PREFIX,ssid);
		nvram_set("wl1.1_ssid",tmp);
		free(ssid);
	}
	else
		nvram_set("wl1.1_ssid","");


	if ((psk=gen_hash_string(2,src,lens)))
	{
		nvram_set("wl1.1_wpa_psk",psk);
		free(psk);
	}
	else
		nvram_set("wl1.1_wpa_psk","");

}

void 
get_mesh_ie(char *part1, char *part2)
{
	char *mesh=NULL;
	char cipher[100];
	...
	if ((mesh=gen_hash_string(1,cipher,strlen(nvram_get("cfg_group")))))
	{
		strncpy(part1,mesh,8);
		strcpy(part2,mesh+8);
		free(mesh);
	}
	else
...
void renew_bh(void)
{
	...
	_dprintf("cfg_key=%s, ssid=%s, psk=%s\n",nvram_get("cfg_group"),nvram_get("wl1.1_ssid"),nvram_get("wl1.1_wpa_psk"));
	nvram_commit();
	restart_wireless();
}
#endif
