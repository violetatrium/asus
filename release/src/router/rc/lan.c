...
#define CP_IFINDEX 2
	...
	if(sw_mode() == SW_MODE_REPEATER){
		for (i = 0; i < MAX_NR_WL_IF; ++i) {
			...
			if ((i==WL_5G_BAND && wlc_band==WL_5G_2_BAND) || (i==WL_5G_2_BAND && wlc_band==WL_5G_BAND))
				;
			else if (i != wlc_band)
				continue;
#else
...
#if defined(RTCONFIG_HIDDEN_BACKHAUL)
		if( sw_mode()!=SW_MODE_REPEATER && strlen(nvram_safe_get("cfg_group")))
		{
...
#endif
#if defined(MAPAC1300) || defined(MAPAC2200) || defined(MAPAC2200P)
...
void start_lan_wlport(void)
{
	...
	foreach (word, nvram_safe_get("wl_ifnames"), next) {
		...
		if (nvram_match(strcat_r(prefix, "radio", tmp), "0"))
        	...
        	if (sw_mode()==SW_MODE_REPEATER 
			&& ((unit==WL_5G_BAND && wlc_band==WL_5G_2_BAND) || (unit==WL_5G_2_BAND && wlc_band==WL_5G_BAND)))
			set_radio(1, unit, 0);
#endif
		if(unit!=wlc_band) set_radio(1, unit, 0);
...
void stop_lan_wlport(void)
{
	...
	foreach (word, nvram_safe_get("wl_ifnames"), next) {
		SKIP_ABSENT_BAND_AND_INC_UNIT(unit);
			...
			set_radio(1, unit, 0);
#endif
		if(unit!=wlc_band) set_radio(0, unit, 0);
		for (subunit = 1; subunit < 4; subunit++)
		{
