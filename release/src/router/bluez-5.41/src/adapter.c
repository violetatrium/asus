					...
					uint8_t addr_type)
{
	...
	if (addr_type==BDADDR_BREDR) {
		...
		disconnect_notify(device, 0);
		bonding_attempt_complete(adapter, &addr, addr_type, MGMT_STATUS_DISCONNECTED);
...
//	int conn_len;
	...
	DBG("Client connect Type: %s", ev->addr.type==BDADDR_BREDR?"BR/EDR remove":"LE");
	if (ev->addr.type==BDADDR_BREDR) {
		disconnected_callback(index, length, param, user_data);
		return;
	}
/*
	conn_len = g_slist_length(adapter->connections);
