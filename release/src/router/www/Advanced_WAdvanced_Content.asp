...
var tcode_name = tcode.substring(0,2);
function initial(){
	show_menu();
	register_event();
	array = new Array(7);
	...
	if('<% nvram_get("wl_unit"); %>' == '1' || '<% nvram_get("wl_unit"); %>' == '2'){ // 5GHz up
		...
		if(based_modelid == "RT-AC88N" || based_modelid == "RT-AC88Q" || based_modelid == "BRT-AC828" || based_modelid == "RT-AC58U" || based_modelid == "4G-AC53U" || based_modelid == "RT-AC82U"){
			inputCtrl(document.form.wl_txbf, 1);
		...
		if((!Qcawifi_support && !Rawifi_support) || based_modelid == "RT-AC87U"
		    || based_modelid == "RT-AC82U" || based_modelid == "RT-AC58U" || based_modelid == "4G-AC53U")		// hide on Broadcom platform
	...
	if(based_modelid == "RT-AC82U" || based_modelid == "RT-AC58U" || based_modelid == "4G-AC53U")
...
function applyRule(){
	if(validForm()){
		...
		if(location_list_support){
			...
			|| (orig_region == "" && document.form.location_code.value != tcode_name)){
