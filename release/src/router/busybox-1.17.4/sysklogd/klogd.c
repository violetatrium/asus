...
int klogd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	while (1) {
		...
		while (1) {
			...
			if (*start) {
#if defined(MAPAC1750) /* remove annoying message: udb_ioctl_copy_out: no handler for ioctl 0x7:0x2 */
				if ((memcmp(start, "udb_ioctl_copy_out:", 19)==0) && (memcmp(start+20, "no handler for ioctl 0x7:0x2", 28)==0))
					; /* skip */
				else {
#endif
