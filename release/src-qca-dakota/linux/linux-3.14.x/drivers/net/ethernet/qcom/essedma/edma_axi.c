...
#ifdef CONFIG_ESSEDMA_DUALWAN
enum {
#if defined(CONFIG_RTAC58U)
	LAN1_PORT=4,
	LAN2_PORT=3,
	LAN3_PORT=2,
	LAN4_PORT=1,
#elif defined(CONFIG_4GAC53U)
	LAN1_PORT=3,
	LAN2_PORT=4,
	LAN3_PORT=2,	/* unused */
	LAN4_PORT=1,	/* unused */
	...
	LAN4_PORT=4,
#else
#error "Define LAN ports id by model!!!"
#endif
};
#endif
...
#if 1 /* ASUS */
static int edma_merge_wan_into_lan __read_mostly = 0;
#endif
...
int reduce_rx_ring_size = 0;
module_param(reduce_rx_ring_size, int, 0);
MODULE_PARM_DESC(reduce_rx_ring_size, "reduce rx ring size");
#endif
...
#if 1 /* ASUS */
static int edma_change_merge_wan_into_lan(struct ctl_table *table, int write,
				      void __user *buffer, size_t *lenp,
				      loff_t *ppos)
{
	struct edma_adapter *adapter;
	...
	if (!edma_netdev[EDMA_LAN]) {
		pr_err("Netdevice for default_lan does not exist\n");
		return -1;
	}

	adapter = netdev_priv(edma_netdev[EDMA_LAN]);
	...
	if (write) {
		...
		if (edma_merge_wan_into_lan) {
			adapter->dp_bitmap = 0x3e;
			edma_cinfo->portid_netdev_lookup_tbl[5] = edma_netdev[EDMA_LAN];
		}
		else {
			adapter->dp_bitmap = 0x1e;
			edma_cinfo->portid_netdev_lookup_tbl[5] = edma_netdev[EDMA_WAN];
		}
	}
...
#if 1 /* ASUS */
	{
		.procname	= "merge_wan_into_lan",
		.data		= &edma_merge_wan_into_lan,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= edma_change_merge_wan_into_lan
	},
#endif
	{}
};
...
static int edma_axi_probe(struct platform_device *pdev)
{
	...
	int idx = 0, idx_mac = 0;
#ifdef CONFIG_ESSEDMA_DUALWAN
	int enable_wan2 = 0, wans_lanport = -1, wan2_portid = -1;
	const int portid_mapping[EDMA_MAX_PORTID_SUPPORTED - 1] = {
		LAN1_PORT,
		LAN2_PORT,
		LAN3_PORT,
		LAN4_PORT
	...
	if (strstr(nvram_safe_get("wans_dualwan"), "lan")) {
		enable_wan2 = 1;
		wans_lanport = simple_strtol(nvram_safe_get("wans_lanport"), NULL, 10);
		wan2_portid = portid_mapping[wans_lanport - 1];
	}
#endif
...
#if defined(CONFIG_RTAC58U) || defined(CONFIG_4GAC53U)
	if (reduce_rx_ring_size)
		edma_cinfo->rx_ring_count = EDMA_RX_RING_SIZE_REDUCED;
	else
		edma_cinfo->rx_ring_count = EDMA_RX_RING_SIZE_DEFAULT;
#else
...
#if defined(CONFIG_RTAC58U) || defined(CONFIG_4GAC53U)
			r = mtd->_read(mtd, 0x1006, ETH_ALEN, &ret_len, edma_netdev[EDMA_WAN]->dev_addr);
			r = mtd->_read(mtd, 0x5006, ETH_ALEN, &ret_len, edma_netdev[EDMA_LAN]->dev_addr);
#ifdef CONFIG_ESSEDMA_DUALWAN
			if (enable_wan2)
				r = mtd->_read(mtd, 0x1006, ETH_ALEN, &ret_len, edma_netdev[EDMA_WAN2]->dev_addr);
#endif
#else
#ifdef CONFIG_RTAC82U
			r = mtd->_read(mtd, 0x1006, ETH_ALEN, &ret_len, edma_netdev[EDMA_WAN]->dev_addr);
			r = mtd->_read(mtd, 0x9006, ETH_ALEN, &ret_len, edma_netdev[EDMA_LAN]->dev_addr);
#ifdef CONFIG_ESSEDMA_DUALWAN
			if (enable_wan2)
				r = mtd->_read(mtd, 0x1006, ETH_ALEN, &ret_len, edma_netdev[EDMA_WAN2]->dev_addr);
#endif
#else
#error "Define MAC offset by model!!!"
#endif
#endif
			put_mtd_device(mtd);
		}
	...
	for (i = 0; i < edma_cinfo->num_gmac; i++) {
		int k, m;
#ifdef CONFIG_ESSEDMA_DUALWAN
		if (!enable_wan2 && i == EDMA_WAN2) /* shouldn't happen */
			continue;
#endif
...
#ifdef CONFIG_ESSEDMA_DUALWAN
	if (enable_wan2) {
		if ( wan2_portid > EDMA_MAX_PORTID_SUPPORTED )
			pr_err("Invalid wan2_portid:%d!!\n", wan2_portid);
		else
			edma_cinfo->portid_netdev_lookup_tbl[wan2_portid] = edma_netdev[EDMA_WAN2]; /* overwrite */
		adapter[EDMA_WAN2]->default_vlan_tag = EDMA_WAN2_DEFAULT_VLAN;
		adapter[EDMA_WAN2]->poll_required = adapter[EDMA_WAN]->poll_required;
		adapter[EDMA_WAN2]->phy_mdio_addr = wan2_portid - 1;
		adapter[EDMA_WAN2]->forced_speed = adapter[EDMA_WAN]->forced_speed;
		adapter[EDMA_WAN2]->forced_duplex = adapter[EDMA_WAN]->forced_duplex;
		adapter[EDMA_WAN2]->dp_bitmap = 1 << wan2_portid;
		snprintf(adapter[EDMA_WAN2]->phy_id, MII_BUS_ID_SIZE + 3, PHY_ID_FMT,
			miibus->id, adapter[EDMA_WAN2]->phy_mdio_addr);
	}
#endif
